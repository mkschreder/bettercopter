...
#ifndef _RG_CSCREENQUADSCENENODE_
#define _RG_CSCREENQUADSCENENODE_
...
public:
			...
			s32 userData)
	{
		...
		int tId = 0;
		services->setPixelShaderConstant("RTScene", (const float*)&tId, 1);
...
void Q3Player::create ( IrrlichtDevice *device, IQ3LevelMesh* mesh, ISceneNode *mapNode, IMetaTriangleSelector *meta )
{
	...
	printf("Creating camera scene node...\n"); 
	...
	ortho = smgr->addCameraSceneNode(0, vector3df(0,0,-0.5), vector3df(0,0,0)); 
	matrix4 orthoMatrix;
    orthoMatrix.buildProjectionMatrixOrthoLH(256, 256, 1.f, 1000.f);
    ortho->setProjectionMatrix(orthoMatrix); 
...
public:
	...
	s32 loadShaders(const std::string &vs, const std::string &ps); 
...
s32 CQuake3EventHandler::loadShaders(const std::string &vs, const std::string &ps)
{
	IVideoDriver * driver = Game->Device->getVideoDriver();
    ...
    if(gpu)
    {
        MyShaderCallBack* shader_callback = new MyShaderCallBack();
        materialId = gpu->addHighLevelShaderMaterialFromFiles(
            vs.c_str(), "main", video::EVST_VS_1_1,
            ps.c_str(), "main" , video::EPST_PS_1_1,
...
/* Load new map
*/
video::ITexture* mainTarget = 0, *rtt0 = 0, *rtt1 = 0, *temp = 0;
    CScreenQuadSceneNode *screenQuad = 0, *screenQuad2 = 0, *screenQuad3 = 0;
    s32 matHorBlur = 0, matVerBlur = 0;
	...
	IVideoDriver * driver = Game->Device->getVideoDriver();
        ...
        if(driver->queryFeature(video::EVDF_RENDER_TO_TARGET))
        {
            if(driver->queryFeature(video::EVDF_MULTITEXTURE))
            {
                ...
                {
                    matHorBlur = loadShaders("horgblur.vert", "horgblur.frag");
                    matVerBlur = loadShaders("vertgblur.vert", "vertgblur.frag");

                    mainTarget = driver->addRenderTargetTexture(driver->getScreenSize(),"mainTarget");
                    rtt0 = driver->addRenderTargetTexture(core::dimension2du(256,256),"rtt0");
                    rtt1 = driver->addRenderTargetTexture(core::dimension2du(128,128),"rtt1");
                    ...
                    driver->setFog(video::SColor(255,0,0,0),video::EFT_FOG_LINEAR,1,60,0.5);
                }
            }
        }
}
		...
		if ( eve.GUIEvent.Caller == gui.MapList && eve.GUIEvent.EventType == gui::EGET_LISTBOX_SELECTED_AGAIN )
		{
			...
			if ( selected >= 0 )
			{
				...
				if ( 0 == MapParent || loadMap != Game->CurrentMapName )
				{
					...
					char rttname[256]; 
					sprintf(rttname, "RTT%d", gamec++); 
					printf("Using rtt: %s\n", rttname); 
					auxRender = Game->Device->getVideoDriver()->
						addRenderTargetTexture(core::dimension2d<u32>(256,256), rttname);
...
void CQuake3EventHandler::Render()
{
	...
	static s32 postType = 0;
	video::IGPUProgrammingServices* gpu = driver->getGPUProgrammingServices();
	if(!postType){
		MyShaderCallBack* mc = new MyShaderCallBack();
		
		postType = gpu->addHighLevelShaderMaterialFromFiles(
	...
	driver->setRenderTarget(auxRender, true, true, video::SColor(0,0,0,255));
	...
	irr::core::matrix4 projortho;
	irr::core::matrix4 oldproj = camera->getProjectionMatrix(); 
	
	
	projortho.buildProjectionMatrixOrthoLH(800, 600, 0, 5000);
	camera->setProjectionMatrix(projortho);
	Game->Device->getSceneManager()->drawAll();
	Game->Device->getGUIEnvironment()->drawAll();
	
	// create triangle
	core::triangle3df tri[2]; 
	tri[0].pointA = core::vector3df(0, 0, -1); 
	tri[0].pointB = core::vector3df(128, 256, -1); 
	tri[0].pointC = core::vector3df(256, 0, -1); 
	...
	material.ZBuffer = 0; 
	material.TextureLayer[0].Texture = auxRender;
	//material.TextureLayer[0].TextureWrap = irr::video::ETC_CLAMP;

	//material.MaterialType = (irr::video::E_MATERIAL_TYPE)postType;
	
	driver->setMaterial(material);
	driver->draw3DTriangle(tri[0], irr::video::SColor(255, 255, 255, 255)); 
	camera->setProjectionMatrix(oldproj); 
	/*
    
    core::matrix4 oldmat; 
	core::matrix4 projmat = projmat.buildProjectionMatrixOrthoLH(800, 600, 0, 5000);
	
	scene::ICameraSceneNode* camera = smgr->getActiveCamera();
	oldmat = camera->getProjectionMatrix(); 
	 */
	 
	//if(!camera) camera = smgr->addCameraSceneNode();
	//camera->setProjectionMatrix(projmat, true);
	/*
	core::position2d<s32> position(50,50); 
	core::rect<s32> sourceRect = core::rect<s32>(0,0,256,256); 
	irr::f32 rotation = 0; 
	irr::video::SColor color(255, 255, 255, 255); 
	
	irr::core::position2d<irr::s32> rotationPoint(auxRender->getSize().Width / 2, auxRender->getSize().Height / 2);
	...
	corner[1] = irr::core::vector2df(position.X+sourceRect.getWidth(),position.Y);
	corner[2] = irr::core::vector2df(position.X,position.Y+sourceRect.getHeight());
	corner[3] = irr::core::vector2df(position.X+sourceRect.getWidth(),position.Y+sourceRect.getHeight());
	...
	for(int x = 0; x < 4; x++)
	{
	  float uvX = uvCorner[x].X/(float)auxRender->getSize().Width;
	  float uvY = uvCorner[x].Y/(float)auxRender->getSize().Height;
	...
	for(int x = 0; x < 4; x++)
	{
	  vertices[x].Pos = irr::core::vector3df(corner[x].X,corner[x].Y,1);
	...
	core::matrix4 projmat = projmat.buildProjectionMatrixOrthoLH(256, 256, 0, 5000);
	ortho->setProjectionMatrix(projmat); 
	
	smgr->setActiveCamera(ortho); 
	//driver->setMaterial(material);
	//driver->setTransform(ETS_WORLD, matrix4()); 
	...
	driver->draw3DLine(
           vector3df(0, 0.f, 0.f),
            vector3df(0, 20.f, 0.f),
            SColor(255, 255, 255, 255)); 
	smgr->setActiveCamera(camera); 
	*/
	driver->draw2DImage(auxRender, core::position2d<s32>(50,50),
    ...
    // second glow color intensity
    video::SColor colors2[] =
    {
        video::SColor(255,80,80,80),
        video::SColor(255,80,80,80),
        video::SColor(255,80,80,80),
        video::SColor(255,80,80,80)
    };
	
	// render normally to a texture
	if(rtt0 && rtt1){
		core::dimension2d<u32> screenSize = driver->getScreenSize(); 
		driver->setRenderTarget(mainTarget,true,true,video::SColor(255,128,160,160));
		smgr->drawAll();
		// resize the normal render
		driver->setRenderTarget(rtt0,true,true,video::SColor(0,0,0,0));
		driver->draw2DImage(mainTarget,core::rect<s32>(0,0,256,256),core::rect<s32>(0,0,screenSize.Width, screenSize.Height),0,colors);
		// copy previous rtt to a lower resolution
		driver->setRenderTarget(rtt1,true,true,video::SColor(0,0,0,0));
		driver->draw2DImage(rtt0,core::rect<s32>(0,0,128,128),core::rect<s32>(0,0,256,256),0,colors2);
