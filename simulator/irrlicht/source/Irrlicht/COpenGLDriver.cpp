		...
		(resetAllRenderStates|| lastmaterial.BlendOperation != material.BlendOperation))
	{
		...
		else
		{
			...
			case EBO_MAX_ALPHA:
				...
				break;
			default:
				printf("Setting blend mode to add..\n"); 
...
void COpenGLDriver::assignHardwareLight(u32 lightIndex)
{
	...
	case video::ELT_SPOT:
		...
		glLightf(lidx, GL_SPOT_EXPONENT, light.OuterCone);
		glLightf(lidx, GL_SPOT_CUTOFF, light.Falloff);
	break;
	case video::ELT_POINT:
		// set position
		//printf("Light position: %f %f %f\r", light.Position.X, light.Position.Y, light.Position.Z); 
		data[0] = light.Position.X;
		data[1] = light.Position.Y;
		data[2] = light.Position.Z;
		data[3] = 1.0f; // 1.0f for positional light
